...
class ModifyNicknameForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ModifyNicknameForm, self).__init__(*args, **kwargs)
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise forms.ValidationError('你已经绑定过了')
    ...
    def clean_verification_code(self):
        ...
        if verification_code == '':
            raise forms.ValidationError('验证码不能为空')
        if not ConfirmString.objects.filter(code=verification_code).exists():
            raise forms.ValidationError('验证码不正确!')
        confirm = ConfirmString.objects.get(code=verification_code)
        c_time = confirm.c_time
        # now = datetime.datetime.now()
        now = timezone.now()
        if now > c_time + datetime.timedelta(settings.CONFIRM_DAYS):
            confirm.delete()
            raise forms.ValidationError('您的邮件已经过期！请重新注册!')
        else:
            confirm.delete()
        return verification_code
...
class ModifyPasswordForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        if 'user' in kwargs:
            self.user = kwargs.pop('user')
        super(ModifyPasswordForm, self).__init__(*args, **kwargs)
